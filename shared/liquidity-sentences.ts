...
// ── 04 누가 샀나 ──
export function s4(w: WhoBought): { headline: string[]; summary: string; caution: string } {
  const caution = `${josa(fmt.amount(w.totalReported), "은는")} 새 빚이 아닙니다`; // '읽을 때 주의' 제목 — 억은/조는(Codex 4차 F4)
  if (!w.top) return { headline: ["이 기간에 집계된 입찰이 없습니다."], summary: "이 기간에 집계된 입찰이 없습니다.", caution };
  ...
  return { headline, summary: `새로 찍은 국채는 ${w.majority ? "절반 이상을" : "가장 많은 몫을"} ${josa(subj, "이가")} 받아갔습니다.`, caution };
}
