...
### 외부 모니터 연결 시 레이아웃 안정성 (2026-09-11)

- 메모·표 위치를 매 렌더에서 `getBoundingClientRect()`로 재측정하던 경로는 좌표가 반올림 경계를 오가면 React #185를 일으킬 수 있었습니다. 실제 컴포넌트에 0.49/0.51px 좌표 진동을 주면 52회 측정 뒤 `Maximum update depth exceeded`가 재현됐습니다.
- 위치·높이는 같은 CSS 레이아웃 좌표(`offsetTop`/`offsetHeight`)로 계산합니다. 콘텐츠 크기와 창 크기가 바뀔 때 프레임당 한 번 측정하며, 직접 설정하는 메모 열 높이를 다시 관찰하지 않습니다. 우측 그래프 폭도 매 렌더 대신 크기 변화로 갱신합니다.
- 수정 후 동일 재현 조건 및 deviceScaleFactor 1/1.25/1.5/2, 1280~2560px 창 너비 전환과 resize 이벤트 100회 연속 발생에서 오류 없음. 실제 모니터 연결·해제 및 해당 노트북의 Chrome/드라이버 조합은 별도 확인이 필요합니다.
...
- 노드 본문·메모·표 셀/제목·사건 제목은 칸 밖 클릭/Tab 이동 시 확정하여 기기 초안(IndexedDB)과 서버에 저장합니다. 타이핑 중에는 해당 편집 컴포넌트의 상태만 바뀝니다. 인사이트·메타 카드는 기존 600ms 자동 저장을 유지합니다. 날짜·기간은 날짜 패널 밖 이동 또는 **저장** 버튼, 표 너비는 드래그 종료 시 확정합니다.
...
- 다른 창의 변경은 5초 주기로 확인합니다. 노드 칸 편집 중에는 해당 카드의 수신/이전 저장 응답 반영을 미뤄 비교 기준을 유지하고, blur에서 작성본을 확정한 뒤 병합·충돌을 처리합니다. 편집 UI 제거 시에도 작성본을 확정합니다. 칸을 벗어나기 전 강제 종료까지 복구를 보장하지는 않습니다.
