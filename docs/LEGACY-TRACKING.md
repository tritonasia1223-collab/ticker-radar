> 보류 모듈의 과거 안내입니다. 2026-09-09 기준 메인 기능과 실행 방법은 [프로젝트 README](../README.md)를 참고하세요. 아래 API·화면·제한 설명은 작성 당시 기록이며, 실행 명령은 현재 package.json이 기준입니다.
...
- **종목 발견** (`/`) — 발굴 대시보드
  - **미장/국장 토글** · 우상단 **데이터 최신화 배지**(오래되면 '갱신 필요') · **[↻ 갱신] 버튼**(GitHub Actions 수집 트리거 — [setup](COLLECT-BUTTON-SETUP.md))
...
- **자본주의 경제사** (`/capitalism`) — 전후 달러 패권사를 인과 플로우 타임라인 + 전 구간 FRED 거시지표 그래프 + 사건별 인사이트로 직접 편집·열람 (아래 별도 섹션 · [아키텍처](CAPITALISM.md))
...
2. 수집 실행 (비용은 Apify 사용량):
   - 배포 앱: 종목 발견 우상단 **[↻ 갱신] 버튼** (GitHub Actions 트리거 — [setup](COLLECT-BUTTON-SETUP.md))
...
| `GH_DISPATCH_TOKEN` |  | — | 배포 앱 [↻ 갱신] 버튼용 — GitHub Actions 트리거 (Vercel 환경변수, [setup](COLLECT-BUTTON-SETUP.md)) |
...
> **상세 아키텍처: [docs/CAPITALISM.md](CAPITALISM.md)** (데이터 모델·API·그래프·인사이트·컴포넌트 지도).
