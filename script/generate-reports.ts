// Generate "왜 뜨나" news reports for surging stocks via web-grounded LLM search.
...
//   npm run reports
//
// 제공자: .env 에 ANTHROPIC_API_KEY 가 있으면 Claude(web_search), 없으면 Gemini(google_search).
//   ANTHROPIC_API_KEY=...   (선택) ANTHROPIC_MODEL=claude-sonnet-4-6
//   GEMINI_TOKEN=...        (선택) GEMINI_MODEL=gemini-2.5-flash
...
// Anthropic(Claude web_search) 제공자 — 키가 있으면 Gemini 대신 사용.
const ANTHROPIC_KEY = process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_TOKEN || "";
const ANTHROPIC_MODEL = process.env.ANTHROPIC_MODEL || "claude-sonnet-4-6";
const PROVIDER = ANTHROPIC_KEY ? "anthropic" : "gemini";
const MODEL_LABEL = PROVIDER === "anthropic" ? ANTHROPIC_MODEL : MODEL;
...
- **서론·맺음말·마크다운 제목(#, ##)·표 없이, 위 '형식' 그대로만** 출력하세요. 바로 "한줄요약:"으로 시작.
- 호재·악재 각 최대 3개, 각 항목 1줄. 전체 8줄 이내.`;
}
...
// Anthropic Messages API + web_search 도구 — Claude가 웹 검색 후 출처 인용과 함께 요약.
async function anthropic(name: string, symbol: string, market: string): Promise<{ summary: string; sources: { title: string; url: string }[] } | null> {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{ role: "user", content: prompt(name, symbol, market) }],
    }),
  });
  if (!res.ok) { console.error(`  ✗ ${symbol}: ${res.status} ${(await res.text()).slice(0, 160)}`); return null; }
  const j = (await res.json()) as any;
  const blocks: any[] = j?.content || [];
  const summary = blocks.filter((b) => b.type === "text").map((b) => b.text).join("").trim();
  if (!summary) return null;
  // 출처: 인용(citations) 우선, 부족하면 검색결과(web_search_tool_result) 보강.
  const seen = new Set<string>();
  const sources: { title: string; url: string }[] = [];
  const push = (title: any, url: any) => {
    const u = String(url || "").trim();
    if (u && !seen.has(u)) { seen.add(u); sources.push({ title: String(title || u).trim(), url: u }); }
  };
  for (const b of blocks) if (b.type === "text" && Array.isArray(b.citations)) for (const c of b.citations) push(c.title, c.url);
  for (const b of blocks) if (b.type === "web_search_tool_result" && Array.isArray(b.content)) for (const r of b.content) if (r?.type === "web_search_result") push(r.title, r.url);
  return { summary, sources: sources.slice(0, 8) };
}

async function genReport(name: string, symbol: string, market: string) {
  return PROVIDER === "anthropic" ? anthropic(name, symbol, market) : gemini(name, symbol, market);
}

async function run() {
  if (PROVIDER === "anthropic" ? !ANTHROPIC_KEY : !KEY) {
    console.error("ANTHROPIC_API_KEY 또는 GEMINI_TOKEN 가 .env 에 없습니다.");
    process.exit(1);
  }
  ...
  console.log(`레포트 생성 대상 ${list.length}개 (신규 ${newcomerCount} + 메이저 ${list.length - newcomerCount}, ${PROVIDER}:${MODEL_LABEL}) …`);
  ...
  for (let i = 0; i < list.length; i++) {
    const t = list[i];
    try {
      const r = await genReport(t.name, t.symbol, t.market);
      if (r) {
        await sql`
          insert into reports (symbol, summary, sources, model, generated_at)
          values (${t.symbol}, ${r.summary}, ${JSON.stringify(r.sources)}, ${MODEL_LABEL}, ${Date.now()})
    ...
    await sleep(PROVIDER === "anthropic" ? 1200 : 4500); // Gemini 무료 RPM 여유 / Anthropic는 짧게
  }
