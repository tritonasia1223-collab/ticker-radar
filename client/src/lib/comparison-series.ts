...
export interface CompareSeriesDef { id: string; label: string; unit: string; color: string; cadence: number; note: string; url: string }
export const COMPARE_SERIES: CompareSeriesDef[] = [
  ...PANELS.map(p => ({ id: p.series, label: p.id === "dollar" ? "달러지수 (접합 계열)" : p.id === "trade" ? "실질 순수출" : p.id === "sp500" ? "미국 주가지수 (OECD)" : p.label,
    unit: p.id === "trade" ? "십억 2017달러·연율" : p.unit, color: p.color,
  ...
  { id: "fx_krw", label: "원/달러 환율", unit: "원 / 1달러", color: "#f472b6", cadence: 1, note: "월평균 · 상승=달러 강세/원화 약세 · 완료된 월만 수록", url: "https://fred.stlouisfed.org/series/EXKOUS" },
  { id: "fx_jpy", label: "엔/달러 환율", unit: "엔 / 1달러", color: "#a78bfa", cadence: 1, note: "월평균 · 상승=달러 강세/엔화 약세 · 완료된 월만 수록", url: "https://fred.stlouisfed.org/series/EXJPUS" },
];
