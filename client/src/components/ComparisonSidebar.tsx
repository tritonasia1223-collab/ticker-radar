...
function ReferencePanel(p: Props) {
  ...
  const matching = p.flows.filter(f => (!p.contextIds.length || p.contextIds.includes(f.slug)) && (plain(f.title) + " " + f.nodes.map(n => plain(n.text)).join(" ")).toLowerCase().includes(search.toLowerCase()));
  const saved = p.nodes.filter(n => (!p.contextIds.length || p.contextIds.includes("plot:" + n.id)) && (n.title + " " + (p.flows.find(f => f.slug === n.flowSlug)?.title ?? "")).toLowerCase().includes(search.toLowerCase()));
  return <div className="space-y-4 p-4" data-testid="comparison-reference">
    ...
    {!!p.contextIds.length && <button className={button} onClick={p.onClearContext}>모든 참고 자료 보기</button>}
    {!!saved.length && <section><h3 className="mb-2 text-xs font-semibold">기존에 가져온 사건 · {saved.length}</h3>{saved.map(n => {
      ...
      return <details key={n.id} className="mb-2 rounded-lg border border-amber-500/20 p-3"><summary className="cursor-pointer break-words text-xs font-medium">{n.title}<span className="mt-1 block text-[10px] font-normal text-muted-foreground">{n.date ?? "날짜 미지정"}{n.endDate && " ~ " + n.endDate}</span></summary><div className="mt-3">{source ? <ReferenceNode node={source} onJump={p.onJump} /> : <p className="text-xs text-muted-foreground">원본 없음 · 기존 제목과 날짜는 보관되어 있습니다.</p>}{flow && <button className={button + " mt-3"} onClick={() => p.onJump(flow.slug)}>원본 카드<ArrowUpRight size={12} /></button>}</div></details>;
    })}</section>}
    <section><h3 className="mb-2 text-xs font-semibold">경제사 카드 · {matching.length}</h3>{matching.map(f => <details key={f.slug} className="mb-2 rounded-lg border p-3" open={p.contextIds.length === 1 && p.contextIds[0] === f.slug || undefined}><summary className="cursor-pointer break-words text-xs font-medium"><span className="mb-1 block text-[10px] font-normal tabular-nums text-muted-foreground">{f.date}</span>{plain(f.title)}</summary><div className="mt-4 space-y-4">{f.nodes.map(n => <ReferenceNode key={n.id} node={n} onJump={p.onJump} />)}<button className={button} onClick={() => p.onJump(f.slug)}>원본 카드<ArrowUpRight size={12} /></button></div></details>)}</section>
