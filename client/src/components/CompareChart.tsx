...
import { lineSegments, simplifyExtrema, moveRange, zoomRange, type ComparePoint, type SavedInsight, type TrendSection } from "../../../shared/cap-comparison";
...
export const CompareChart = memo(function CompareChart({ series, range, extent, indexed, onRange, phases, events, onEvents, simplifyMonths, notes, selectedNote, onNote, onCreate, tool, onCancelTool, resetAxes, layoutKey }: {
  ...
  useEffect(() => {
    ...
    const m = frameMeasurement(() => { setWidth(Math.max(420, el.clientWidth)); setChartHeight(clamp(window.innerHeight - Math.max(0, el.getBoundingClientRect().top) - 180, 330, 720)); });
  ...
  const plotTop = 76, plotBottom = chartHeight - (events.length ? 85 : 43), plotHeight = plotBottom - plotTop;
  ...
  wheel.current = e => {
    ...
    const side = px < left ? shapes[0] : px > right && !indexed ? shapes[1] : undefined;
  ...
  return <div ref={host} className="relative min-w-[420px]" data-testid="compare-chart">
    ...
    <svg ref={svg} width="100%" height={chartHeight} viewBox={"0 0 " + width + " " + chartHeight} className="touch-none select-none outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-sky-500/30" tabIndex={0} aria-label="시간 기준 인사이트와 거시지표 비교 그래프"
      ...
      onPointerDown={e => {
        ...
        const side = px < left ? shapes[0] : px > right && !indexed ? shapes[1] : undefined;
        const part = side ? "value" : py > plotBottom ? "time" : px >= left && px <= right ? "plot" : "none";
        if (part === "none") return;
      ...
      <defs><clipPath id={clip}><rect x={left} y={plotTop} width={span} height={plotHeight} /></clipPath></defs>
      ...
      <rect x={left} y={plotBottom} width={span} height={35} fill="transparent" style={{ cursor: "ew-resize" }} />
      ...
      {Array.from({ length: 7 }, (_, i) => { const time = from + (to - from) * i / 6, px = x(time); return <g key={i} pointerEvents="none"><line x1={px} x2={px} y1={plotTop} y2={plotBottom} stroke="currentColor" opacity={.07} /><text x={px} y={plotBottom + 23} textAnchor="middle" fontSize={10} fill="currentColor" opacity={.65}>{iso(time).slice(0, to - from > 5 * 365 * DAY ? 4 : 7)}</text></g>; })}
      ...
      {cursor !== null && <line x1={x(cursor)} x2={x(cursor)} y1={plotTop} y2={plotBottom} stroke="currentColor" strokeDasharray="3 4" opacity={.3} pointerEvents="none" />}
...
function Navigator({ extent, range, onRange, series, cadence, simplifyMonths }: { extent: Domain; range: Domain; onRange: (r: Domain) => void; series: ComparePoint[]; cadence: number; simplifyMonths: number }) {
  const drag = useRef<{ x: number; range: Domain; part: string } | null>(null);
  ...
  const paths = lineSegments(series, cadence).map(g => simplifyExtrema(g, simplifyMonths)).map(g => g.map((p, i) => (i ? "L" : "M") + px(p.time) + "," + (36 - (p.value - lo) / (hi - lo || 1) * 28)).join(" "));
  return <div className="border-t px-4 py-2"><div className="mb-1 flex justify-between text-[10px] text-muted-foreground"><span>전체 기간 · 양 끝 조절 / 선택 영역 이동</span><button className="underline" onClick={() => onRange(extent)}>전체 보기</button></div>
    <svg viewBox="0 0 1000 44" width="100%" height={44} className="touch-none rounded bg-muted/25" aria-label="전체 기간 탐색" data-testid="compare-navigator"
      onPointerDown={e => { if (e.button !== 0) return; const box = e.currentTarget.getBoundingClientRect(); drag.current = { x: (e.clientX - box.left) / box.width, range, part: (e.target as Element).getAttribute("data-part") ?? "move" }; e.currentTarget.setPointerCapture(e.pointerId); }}
      onPointerMove={e => { if (!drag.current) return; const box = e.currentTarget.getBoundingClientRect(), d = drag.current, delta = ((e.clientX - box.left) / box.width - d.x) * span; const r: Domain = d.part === "start" ? [Math.max(a, Math.min(d.range[1] - DAY * 31, d.range[0] + delta)), d.range[1]] : d.part === "end" ? [d.range[0], Math.min(b, Math.max(d.range[0] + DAY * 31, d.range[1] + delta))] : moveRange(d.range, delta, extent); onRange(r); }}
      ...
      <rect x={px(range[0])} y={2} width={Math.max(1, px(range[1]) - px(range[0]))} height={40} fill="#38bdf8" fillOpacity={.12} stroke="#38bdf8" data-part="move" style={{ cursor: "grab" }} />
      {(["start", "end"] as const).map((part, i) => <rect key={part} x={clamp(px(range[i]) - 6, 0, 988)} y={2} width={12} height={40} rx={3} fill="#38bdf8" data-part={part} style={{ cursor: "ew-resize" }} />)}
    </svg><div className="flex justify-between text-[10px] text-muted-foreground"><span>{iso(a)}</span><span>{iso(b)}</span></div>
  </div>;
}
