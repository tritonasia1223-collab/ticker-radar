...
export function ComparisonInsightContext({ note, currentContext, seriesData, canEdit, onRestore, children }: {
  ...
  return <div className="space-y-4">
    <section className="rounded-lg border bg-muted/10 p-3" aria-label="인사이트 관련 그래프">
      ...
      {canEdit && <details className="mt-3 text-[11px]"><summary className="cursor-pointer text-muted-foreground">관련 그래프 수정</summary><button className="my-2 rounded border px-2 py-1" onClick={() => save({ context: { ids: [...currentContext.ids], spread: currentContext.spread ? { ...currentContext.spread } : null } })}>{context ? "현재 표시 그래프로 바꾸기" : "현재 그래프 연결"}</button><div className="grid grid-cols-1 gap-1.5">{COMPARE_SERIES.map(s => {
