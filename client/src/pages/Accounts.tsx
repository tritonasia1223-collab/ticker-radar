...
export default function Accounts() {
  ...
  const add = useMutation({
    mutationFn: async (h: string) => (await apiRequest("POST", "/api/accounts", { handle: h })).json(),
  ...
  const addBulk = useMutation({
    mutationFn: async (handles: string[]) => {
      ...
      for (const h of handles) {
        try { await apiRequest("POST", "/api/accounts", { handle: h }); ok++; } catch { fail++; }
      }
      return { ok, fail };
    },
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <header className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">트윗을 수집할 X(트위터) 핸들을 관리합니다. 비활성 계정은 수집에서 제외됩니다.</p>
      </header>
      ...
      <Card className="p-4 mb-4">
        ...
        <div className="mt-3">
          <textarea
            ...
            placeholder="여러 핸들 일괄 추가 — 줄바꿈 또는 쉼표로 구분"
